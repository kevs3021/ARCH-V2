// app/api/breakdowns/[breakdownId]/reject/route.ts
...
export async function PATCH(
  ...
  // 3. Fetch all breakdown_approvers for this request to check if any are Rejected
  ...
  // Check if ANY breakdown is rejected (case-insensitive)
  const anyRejected =
    Array.isArray(allApprovers) &&
    allApprovers.length > 0 &&
    allApprovers.some((a: { status: string }) => a.status.toLowerCase() === 'rejected');

  if (!anyRejected) {
    return NextResponse.json({ success: true });
  }

  // 4. At least one breakdown rejected — fetch current request status and history
  ...
  // 5. Update request status to rejected with system history entry (case-insensitive)
